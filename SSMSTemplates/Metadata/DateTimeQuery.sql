...
SELECT ''', [A].[SQL_TABLE_SCHEMA], ''' AS TABLE_SCHEMA, ''', [A].[TABLE_NAME], ''' AS [TABLE_NAME], ''', [A].[MatchedDateColumn], ''' AS COLUMN_NAME, ', [A].[MatchedDateColumn], ' AS COLUMN_VALUE, COUNT_BIG(1) AS ROW_COUNT
FROM ', [A].[SQL_TABLE_SCHEMA], '.', [A].[TABLE_NAME], '
