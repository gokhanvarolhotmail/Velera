...
UNION ALL
SELECT CONCAT('INSERT INTO TEMP
SELECT ''', [A].[SF_TABLE_CATALOG], ''' AS TABLE_CATALOG, ''', [A].[SF_TABLE_SCHEMA], ''' AS TABLE_SCHEMA, ''', [A].[TABLE_NAME], ''' AS TABLE_NAME, ''', [A].[MatchedDateColumn], ''' AS COLUMN_NAME, ', [A].[MatchedDateColumn], ' AS COLUMN_VALUE, COUNT(1) AS ROW_COUNT
