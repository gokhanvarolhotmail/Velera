...
SELECT ''', [SQL_TABLE_SCHEMA], ''' AS TABLE_SCHEMA, ''', [TABLE_NAME], ''' AS [TABLE_NAME], ''', [MatchedDateColumn], ''' AS COLUMN_NAME, ', [MatchedDateColumn], ' AS COLUMN_VALUE, COUNT_BIG(1) AS ROW_COUNT
FROM ', [SQL_TABLE_SCHEMA], '.', [TABLE_NAME], '
GROUP BY ', [MatchedDateColumn], ';

') AS SQL
FROM(SELECT TOP 999999 *
FROM [#ColumnMatches]
WHERE [MatchedDateColumn] IS NOT NULL 
ORDER BY [SQL_TABLE_SCHEMA],[TABLE_NAME]) A
UNION ALL
SELECT '
...
UNION ALL
SELECT CONCAT('INSERT INTO TEMP
SELECT ''', [SF_TABLE_SCHEMA], ''' AS TABLE_SCHEMA, ''', [TABLE_NAME], ''' AS [TABLE_NAME], ''', [MatchedDateColumn], ''' AS COLUMN_NAME, ', [MatchedDateColumn], ' AS COLUMN_VALUE, COUNT(1) AS ROW_COUNT
FROM ', [SF_TABLE_SCHEMA], '.', [TABLE_NAME], '
GROUP BY ', [MatchedDateColumn], ';
